 ...
 * `.result` (never the formula itself, and never evaluated) so nothing
 * derived from a spreadsheet formula is ever treated as trusted input.
 */
 ...
 * supported by exceljs — callers get a clear error asking to re-save as
 * .xlsx or .csv.
 */
export async function parseImportFile(buffer: Buffer, fileName: string): Promise<ParsedImportRow[]> {
  ...
  const requiredFields: ImportField[] = ["systemName", "assetCode"];
  ...
  worksheet.eachRow((row, rowNumber) => {
    ...
    rows.push({
      ...
      input: {
        ...
        allocatedPersonName: values.allocatedPersonName ?? "",
        assetCode: values.assetCode ?? "",
        keyboard: values.keyboard ?? "",
        mousePad: values.mousePad ?? "",
