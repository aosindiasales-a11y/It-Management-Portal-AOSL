...
import { previewSystemImport, confirmSystemImport } from "@/features/systems/import/actions";
import { ACCEPTED_IMPORT_EXTENSIONS, MAX_IMPORT_FILE_SIZE_MB } from "@/features/systems/import/constants";
import type { ImportPreviewResult, ImportResult, ImportRow, ImportRowAction } from "@/features/systems/import/types";

interface ImportSystemsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** Called after a successful (even partial) import so the caller can refresh the systems list. */
  onImported: () => void;
}
...
async function downloadErrorReport(result: ImportResult): Promise<void> {
  ...
  sheet.addRow(["Row", "System Name", "Asset Code", "Allocated Person", "Error"]);
  ...
  result.failures.forEach((failure) => {
    sheet.addRow([
      failure.rowNumber,
      failure.input.systemName || "—",
      failure.input.assetCode || "—",
      failure.input.allocatedPersonName || "—",
      failure.error ?? "",
    ]);
  });
  ...
  triggerDownload(
    ...
    `systems-import-errors-${result.fileName.replace(/\.[^.]+$/, "")}.xlsx`
  );
}

export function ImportSystemsDialog({ open, onOpenChange, onImported }: ImportSystemsDialogProps) {
  ...
  async function handleFile(file: File | undefined | null) {
    ...
    try {
      ...
      const response = await previewSystemImport(formData);
      if (!response.success) {
  ...
  async function handleConfirmImport() {
    ...
    setImporting(true);
    try {
      const response = await confirmSystemImport(
        preview.fileName,
      ...
      <DialogContent className="flex max-h-[85vh] flex-col overflow-hidden sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle>
            ...
            {step === "result" && "Asset Import Complete"}
          </DialogTitle>
          <DialogDescription>
            {step === "upload" && "Upload an Excel or CSV file containing asset records to import multiple systems at once."}
          ...
          <div className="space-y-4 overflow-y-auto">
            ...
            >
              ...
              <p className="text-sm font-medium text-foreground">
                {analyzing ? "Reading and validating your file…" : "Drag & drop your Excel or CSV file here"}
              </p>
              ...
              <table className="w-full min-w-[640px] text-sm">
                <thead className="sticky top-0 bg-secondary/95 backdrop-blur">
                  <tr>
                    ...
                    <th className="whitespace-nowrap px-3 py-2 text-left text-xs font-medium text-muted-foreground">System Name</th>
                    <th className="whitespace-nowrap px-3 py-2 text-left text-xs font-medium text-muted-foreground">Asset Code</th>
                    <th className="whitespace-nowrap px-3 py-2 text-left text-xs font-medium text-muted-foreground">Allocated Person</th>
                ...
                <tbody className="divide-y divide-border">
                  {pageRows.map((row) => (
                    <tr key={row.rowNumber}>
                      ...
                      <td className="max-w-[160px] truncate px-3 py-2">{row.input.systemName || "—"}</td>
                      <td className="max-w-[160px] truncate px-3 py-2">{row.input.assetCode || "—"}</td>
                      <td className="max-w-[160px] truncate px-3 py-2">{row.input.allocatedPersonName || "—"}</td>
                      <td className="max-w-[100px] truncate px-3 py-2">{row.input.status || "—"}</td>
